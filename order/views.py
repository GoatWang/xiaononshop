...
def order_list(request):
    if not request.user.is_authenticated:
        ...
        line_id = line_profile.line_id
        line_profile = LineProfile.objects.get(line_id=line_id)
        current_orders = list(Order.objects.filter(line_profile=line_profile, bento__date__gt=datetime.now()-timedelta(1), bento__delete_time=None).values_list('id', 'number', 'price', 'bento__date', 'bento__name', 'bento__bento_type__bento_type', 'bento__cuisine', named=True).order_by('bento__date'))
